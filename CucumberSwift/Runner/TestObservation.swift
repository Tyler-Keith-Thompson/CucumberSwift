    ...
    public func testBundleDidFinish(_ testBundle: Bundle) {
        DispatchQueue.main.async {
            ...
            if  let documentDirectory = try? FileManager.default.url(for: .documentDirectory, in: .userDomainMask, appropriateFor:nil, create:false),
                let data = try? JSONSerialization.data(withJSONObject: Cucumber.shared.features.taggedElements(with: Cucumber.shared.environment, askImplementor: true).map { $0.toJSON() }, options: JSONSerialization.WritingOptions.prettyPrinted) {
